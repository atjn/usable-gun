...
export default async function (__usable_environment) {
	...
	Radisk.Radix;
	var AWS;
	await Gun.on("create", async function (root) {
		this.to.next(root);
		var opt = root.opt;
		if (!opt.s3 && !__usable_globalThis.process.env.AWS_S3_BUCKET) {
			return;
		}
		...
		opts.bucket = opts.bucket || __usable_globalThis.process.env.AWS_S3_BUCKET;
		opts.region =
			opts.region || __usable_globalThis.process.env.AWS_REGION || "us-east-1";
		opts.accessKeyId = opts.key =
			opts.key ||
			opts.accessKeyId ||
			__usable_globalThis.process.env.AWS_ACCESS_KEY_ID;
		opts.secretAccessKey = opts.secret =
			opts.secret ||
			opts.secretAccessKey ||
			__usable_globalThis.process.env.AWS_SECRET_ACCESS_KEY;
		if ((opt.fakes3 = opt.fakes3 || __usable_globalThis.process.env.fakes3)) {
	...
	async function Store(opt) {
		...
		if (false !== opt.rfs) {
			await (
				await rfsmixPlugin(__usable_environment)
			)(opt, store);
