...
export default async function (__usable_environment) {
	...
	__usable_module.exports = await (async (a) => {
		async function s3(opt) {
			...
			opt.bucket =
				opt.bucket ||
				opt.Bucket ||
				__usable_globalThis.process.env.AWS_S3_BUCKET;
			opt.region =
				opt.region || __usable_globalThis.process.env.AWS_REGION || "us-east-1";
			...
			s.config = opt;
			s.AWS = (await import("aws-sdk")).default;
			s.on = a.on;
			if (
				(s.config.fakes3 =
					s.config.fakes3 ||
					opt.fakes3 ||
					__usable_globalThis.process.env.fakes3)
			) {
