...
import __import_node_https from "node:https";
import __import_node_http from "node:http";
	...
	(() => {
		...
		Gun.on("opt").event((gun, opt) => {
				...
				((req, res, next) => {
					...
					if (
						...
						res.end(
							(gun.wsp.js =
								gun.wsp.js ||
								__import_node_fs.readFileSync(__usable_dirname + "/../gun.js")),
			...
			if (
				...
				__import_node_https.globalAgent.maxSockets =
					__import_node_http.globalAgent.maxSockets =
