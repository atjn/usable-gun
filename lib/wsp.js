...
export default async function (__usable_environment) {
	...
	await (async () => {
		...
		var ws = (await import("../usableLib/wsWrapper.js")).default.Server;
		var http = await httpPlugin(__usable_environment);
		var url = (await import("node:url")).default;
		await Gun.on("opt").event(async (gun, opt) => {
			gun.__.opt.ws = opt.ws = gun.__.opt.ws || opt.ws || {};
			async function start(server, port, app) {
				if (app && app.use) {
				...
				await (
					await wsPlugin(__usable_environment)
				)(
			...
			if (
				...
				(await import("node:https")).default.globalAgent.maxSockets = (
					await import("node:http")
				).default.globalAgent.maxSockets = gun.__.opt.maxSockets || Infinity;
			}
			gun.wsp.msg =
				gun.wsp.msg ||
				((id) => {
            ...
            */
			...
			var WebSocket = (await import("../usableLib/wsWrapper.js")).default;
